...
class Options(object):
    ...
    def __init__(self):
        self.fields = ["gene", 
                       ...
                       "num_het", 
                       "num_hom_alt"]
        self.where_filters = ["vep_pick = 1 AND filter = None"]
    ...
    def translate_where(self, plaintext_where):
        ...
        standard_transcripts = "(vep_pick = 1 AND filter = None AND ( {exclude} )) OR " \
                               "(filter = None AND {transcriptlist}".format(transcriptlist=self.transcripts, 
